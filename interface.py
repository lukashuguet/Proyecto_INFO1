...
# =============================================================================
# FUNCIONES CONTROLADORAS (BLOQUE 3: OPERACIONES LEBL DENTRO DE LA V4)
# =============================================================================
...
def ejecutar_cargar_vuelos():
    """Carga de forma separada las llegadas (Arrivals.txt) conforme dicta la V4."""
    global lista_llegadas
...
def ejecutar_cargar_salidas():
    """Carga el archivo de salidas (Departures.txt) usando el nuevo módulo."""
...
def ejecutar_simular_hora():
    """Procesa la asignación dinámica para la hora seleccionada por el usuario."""
    global objeto_lebl, lista_movimientos_merged, hora_actual_simulacion, lista_aeropuertos
    if not objeto_lebl or not lista_movimientos_merged:
        return
    ...
    no_asignados = LEBL.AssignGatesAtTime(objeto_lebl, lista_movimientos_merged, string_hora)

    ActualizarDiagramaVisual(objeto_lebl)
    label_status_hora.config(text=f"Hora Activa: {string_hora} | No asignados en este tramo: {no_asignados}")


def cambiar_slider_hora(val):
    """Actualiza de forma reactiva la hora de la simulación al arrastrar el control visual."""
    global hora_actual_simulacion
    hora_actual_simulacion = int(val)
    ejecutar_simular_hora()
...
def ejecutar_plot_ocupacion_completa():
    ...
    if not objeto_lebl or not lista_movimientos_merged:
        ...
        return
    LEBL.PlotDayOccupancy(objeto_lebl, lista_movimientos_merged)
    ...
    for t_idx, terminal in enumerate(bcn.terminals):
        ...
        for area in terminal.boarding_areas:
            ...
            for gate in area.gates:
                ...
                # EMOJI DEL AVIÓN REMOVIDO: Texto directo limpio
                if gate.occupied:
...
# =============================================================================
# CONSTRUCCIÓN DE LA ARQUITECTURA DE LA VENTANA (TKINTER SECTIONS)
# =============================================================================
root = tk.Tk()
...
tk.Label(frame_izquierdo, text="PANEL DE CONTROL GENERAL V4", font=("Arial", 14, "bold"), bg=COLOR_BG,
...
# SECTION 2: Gestión de Base de Datos Mundial (Consistencia de Colores)
...
marco_lebl = tk.LabelFrame(frame_izquierdo, text=" 3. Procesamiento Dinámico LEBL (V4) ", font=("Arial", 10, "bold"),
                           bg=COLOR_BG, fg="#000000", padx=5, pady=5)
...
slider_tiempo = tk.Scale(marco_simulacion, from_=0, to=23, orient="horizontal", tickinterval=4, font=("Arial", 8),
                         command=cambiar_slider_hora, bg=COLOR_BG)
slider_tiempo.pack(fill="x", padx=5, pady=5)

# EMOJI DE LA GRÁFICA REMOVIDO: Texto directo limpio
...
# MONITOR GRÁFICO DERECHO
tk.Label(frame_derecho, text="DIAGRAMA DE OCUPACIÓN COMPLETO EN TIEMPO REAL (V4)", font=("Arial", 13, "bold"),
